...
def get_me(user: dict = Depends(get_current_user)):
    ...
    tenant_name = ""
    if db_user.get("tenant_id"):
        tenant = tenants_col.find_one({"_id": ObjectId(db_user["tenant_id"])})
        if tenant:
    ...
    return {
        ...
        "email": db_user["email"],
        "role": db_user["role"],
        "tenant_id": str(db_user["tenant_id"]) if db_user.get("tenant_id") else "",
...
def update_profile_image(
    ...
    users_col.update_one(
        {"_id": ObjectId(user["user_id"])},
        {"$set": {"profile_image": req.profile_image}}
    )
    return {
